...
def odontologia_dashboard_stats() -> rx.Component:
    """
    📊 Componente principal de estadísticas para dashboard odontológico
    
    Incluye todas las métricas, alertas y información relevante
    para el odontólogo en una vista consolidada.
    """
    return rx.vstack(
        # Alerta de urgentes (solo se muestra si hay)
        alerta_pacientes_urgentes(),
        ...
        rx.hstack(
            # Próxima consulta (50% width)
            rx.box(
                proxima_consulta_info(),
                width="50%"
            ),
            
            # Resumen + acciones (50% width)  
            rx.box(
                rx.vstack(
                    resumen_actividad(),
                    acciones_rapidas(),
